...
export default function Root() {
	const formik = useFormik({
		...
		onSubmit: async ({ email, password }) => {
			...
			await EmailSchema.validate(email).catch(({ errors }) => (formik.errors.email = errors));
			await PasswordSchema.validate(password).catch(({ errors }) => (formik.errors.password = errors));
